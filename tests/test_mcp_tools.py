...
def test_state_candidates_only_include_state_paths():
    result = yang_suggest_gnmi_candidates(
        product="sros", feature_or_query="srv6", kind="state", limit=3
    )

    assert result["candidates"]
    assert all(candidate["kind"] == "state" for candidate in result["candidates"])
